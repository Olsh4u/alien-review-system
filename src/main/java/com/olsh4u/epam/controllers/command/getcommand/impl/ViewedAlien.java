 ...
 */
public class ViewedAlien implements Command {
    ...
    public CommandResponse execute(final HttpServletRequest req,
        ...
        try {
            ...
            int countAlien = ServiceFactory.getInstance().getAlienService().countAllAliensThatUserWatch(userId);
            List<Alien> alienList = ServiceFactory.getInstance()
                    .getAlienService().findAliensThatUserWatch(userId, page, COUNT_ALIEN_IN_MY_SHOWS_PAGE);
            req.setAttribute(ATTRIBUTE_ALIENS, alienList);
            req.setAttribute(PARAMETER_ITEM_ON_PAGE, COUNT_ALIEN_IN_MY_SHOWS_PAGE);
            req.setAttribute(PARAMETER_COUNT_ALL_ALIENS, countAlien);
            return new CommandResponse(RoutingType.FORWARD, ROUTING_MY_ALIENS_JSP, req, resp);
