...
/**
 * Command for the {@link Ability} edit page.
 *
 * @see Command
 * @see com.olsh4u.epam.controllers.command.getcommand.GetCommandProvider
 */
public class EditAbilityGetCommand implements Command {
    ...
    public CommandResponse execute(final HttpServletRequest req,
        ...
        CommandUtils.transferMapAttribute(ATTRIBUTE_ABILITY_PROBLEM, req);
        try {
            int page = req.getParameter(PARAMETER_PAGE) != null ? Integer.parseInt(req.getParameter(PARAMETER_PAGE)) : DEFAULT_PAGE_NUMBER;
            int countAllAbilities = ServiceFactory.getInstance().getAbilityService().countAllAbilities();
            List<Ability> abilities = ServiceFactory.getInstance()
                    .getAbilityService().findAbilitiesPageByPage(page, COUNT_ABILITIES_IN_ADMIN_PAGE);
            req.setAttribute(PARAMETER_ABILITY, abilities);
            req.setAttribute(PARAMETER_COUNT_ALL_ABILITIES, countAllAbilities);
            req.setAttribute(PARAMETER_ITEMS_ON_PAGE, COUNT_ABILITIES_IN_ADMIN_PAGE);
            return new CommandResponse(RoutingType.FORWARD, ROUTING_ADMIN_ABILITY_JSP, req, resp);
